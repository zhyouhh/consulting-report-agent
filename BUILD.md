...
- 该文件不会入库，但会作为打包产物的一部分被复制到 `dist/咨询报告助手/managed_search_pool.json`。
...
**Q: 内置搜索池文件放在哪里？**  
A: 构建机源文件放在项目根目录的 `managed_search_pool.json`；打包后副本会进入 `dist/咨询报告助手/managed_search_pool.json`；运行时状态和缓存写在用户目录下的 `search_runtime_state.json` 与 `search_cache.json`。
