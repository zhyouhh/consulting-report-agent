...
- **手势劫持修复（2026-07-12，试用反馈「选项目后滑动失效 + 上滑变刷新网页」，Codex 2 轮 APPROVED + 部署 kr-web-01 bundle `index-772bqLPQ.js`）**：真因＝选项目后聊天区变成装满内容的垂直滚动容器，横/斜滑被浏览器接管为滚动/overscroll 手势（下拉刷新/历史导航），接管即发 **`touchcancel` 而非 `touchend`** → 手势哑掉。修法是**掐断接管本身**，两道都是触摸设备 scoped、桌面零影响：① 壳根 `style` 加 **`touchAction: 'pan-y pinch-zoom'`**（横向 pan 不再交给浏览器；内层 `.overflow-x-auto` 表格/代码块横滚**不受影响**——touch-action 手势链止于最近的滚动容器；**绝不能加 `pan-x`**）；② `index.css` **`@media (pointer: coarse){html,body{overscroll-behavior:none}}`**（杀下拉刷新 + 水平历史导航；`overscroll-behavior` 只许出现在该块内，守护测试锁死）。**两条铁律（Codex 首轮 BLOCKER，勿改回）**：`onShellTouchCancel` 必须保持**纯清理**——cancel 也代表系统打断（来电/通知栏/多指接管），拿 touchmove 最后坐标补判会在非用户意图时误开关抽屉；**`viewport-fit=cover` 禁止加**，除非全站做齐四边 `env(safe-area-inset-*)`（当时只有 MobileShell 底部有 padding，半吊子 cover 会让顶栏/抽屉/admin 页钻进刘海安全区）。同批顺带：coarse 块内表单控件 `font-size:max(1em,16px)!important`（iOS Safari 聚焦 <16px 输入框自动放大整页，composer 原 15px；`!important` 压 Tailwind 字号类特异性）；viewport meta 加 `interactive-widget=resizes-content`（安卓 Chrome 108+ 键盘弹出收缩布局视口、100dvh 跟着变小，iOS/桌面忽略）；壳根加 `h-screen` 类作 100dvh 解析兜底（老 WebKit inline 声明被丢弃时类生效）。已知支持底线：iOS 16 以下无 `overscroll-behavior`（下拉刷新杀不掉，接受）。回归 `tests/mobileViewport.source.test.mjs`（新）+ `mobileShell.source.test.mjs` 扩充。
