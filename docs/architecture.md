...
本文件保存咨询报告助手的详细实现机制、历史演进与高风险边界。日常 Agent 规则以仓库根
`CLAUDE.md` 为准；改到具体子系统时，再按根文件的指针阅读本文件对应章节。各日期段里的
“待办/未 push/当前”等表述是当时快照，不代表现状；现行状态只看 `docs/current-worklist.md`，
历史流水账看 `docs/worklist-history.md`。

## 项目定位

面向非技术同事的咨询报告 Agent，同时提供多用户 Web 站点和 Windows PyWebView 桌面分发；桌面交付形态是 `dist\咨询报告助手\` 整个文件夹（不是裸 exe）。当前承诺 Web 试用、Windows 分发和 `可审草稿` 导出（2026-07-16 起自带标准咨询排版壳——封面/目录/页眉页码，见「## docx 导出排版模板」段），不承诺 macOS 正式桌面支持和最终排版稿。
...
- **预览滚动容器必须自带 `relative`（2026-07-10 修）**：预览走 rehype-katex + rehypeRaw，KaTeX 的 `.katex-mathml`（position:absolute 无障碍隐藏层）等 absolute 元素以最近 positioned 祖先为 containing block——滚动容器（`overflow-y-auto bg-field` 那个 div）若非 positioned，锚点落到外层面板根 → 逃出 overflow 裁剪 → 含公式的长文件把整页文档撑高出大片空白（试用用户实报：选中报告正文/分析记录后页面下方出现可滚动空白区）。`filePreviewPanel.source.test.mjs` 有 source-guard 锁死，**别删这个 `relative`**。
...
**Settings**（`config.py`）：`managed_vision_model`（默 `deepseek-v4.1-flash`；2026-09-18 前为 `Qwen/Qwen3-VL-8B-Instruct`，config v6 迁移旧默认；服务端只读，`/api/settings` 忽略客户端值）/`vision_enabled`。换视觉模型会改变图片缓存命名空间：旧命名空间经 `legacy_image_cache_namespaces` 在删材料/删项目时一并 release，防孤儿缓存。**依赖**（`requirements.txt`）：`markitdown[docx,pptx,xlsx,xls,pdf]==0.1.6`（**不能用 plan 写的 0.0.1a3，无 `enable_plugins`**）+ `rapidocr-onnxruntime` + `onnxruntime==1.27.0` + `xlrd==2.0.2`；mac 用 `uv pip install`（venv 无 pip）。**限额**常量集中 `backend/material_limits.py`。
...
- `price_micro_yuan(model,hit,miss,completion,pricing,at=None)` 三档计价（token×元每百万token=微元，`round`；未知模型 `FALLBACK_MODEL_PRICING`）。单价经 `unit_prices` 取：`MANAGED_PEAK_PRICE_MULTIPLIER` 登记的模型（2026-09-18 起 `deepseek-v4.1-flash`，表内为 DeepSeek 官方空闲价 0.02/1/4）在官方高峰时段（`is_peak_shanghai`：北京时间周一至周五 9–12、14–18 点，左闭右开，不含法定节假日）×2，按**结算时刻**判定；fail-closed 分支同样走 `unit_prices`。`deepseek-v4-pro` 已于 2026-09-18 退役、不在单价表。
...
**custom 真激活（config.py）**：`normalize_settings_payload` 非 legacy honor `mode`（`config_version < LEGACY_MODE_CONFIG_VERSION`（=4）的 legacy 仍强制 managed 迁移安全；2026-09-18 起 `DESKTOP_CONFIG_VERSION` 已升到 6，两者分离，升版本不会把 v4+ 的 custom 用户误当 legacy）；**`mode` 现持久化**（从 `save_settings` 的剔除清单移除——之前被当运行时派生字段剔掉，导致 custom 选择存盘即丢、活不过一个请求，custom 从未端到端生效）；`managed_base_url` 服务端只读（normalize 强制回 `DEFAULT_MANAGED_BASE_URL` + 不持久化；`SettingsUpdate.managed_base_url` 改 Optional-ignore）。保存 custom 时端点用 `validate_custom_api_base` 即时校验（400）。
...
- **为什么砍"手动把旧材料/旧图片重新挂这一轮"**（验证后的取舍，非纯删功能）：① `read_material_file` 读成功后正文按 `material:{id}` 存进 `conversation_state.json` 工作记忆、每轮注回（见「## 管理型搜索池」上方 N6 + worklist 顶「工作记忆旁路」实证）；② 当时默认 managed 模型 `deepseek-v4-pro` 是纯文本，图片走转写、读过同样留（2026-09-18 起默认 `deepseek-v4.1-flash` 在 `MULTIMODAL_MODEL_MARKERS` 内：当前轮图片材料以 `image_url` 直送主模型，同时 `_warm_image_transcript_async` 后台用 flash 转写入缓存，历史轮次 cache-first 回放转写文字（不新发视觉请求，未就绪则只留材料清单）；聊天框的图片（粘贴/拖入/回形针）与文档一样先入材料库再挂到本轮，前端对剪贴板的 `image.png` 按时间戳重命名、bmp 转 png；transient 图片通道仅为旧前端兼容保留）；③ 模型每轮已从 `build_project_context` 的「## 可用项目材料」拿到全部材料 id。故"重新挂"对默认配置零增量。**后端 `_build_user_content` 的 image_url/transcript 能力与其测试（`test_chat_runtime.py:203/212`）保留不动**，只是前端不再给入口（custom + 真多模态模型想重看原图像素是边角，重新上传即可）。
...
- **抽屉开关 = 顶栏按钮 + 点 scrim + 滑动手势**（任一）。滑动判定纯函数 `utils/drawerSwipe.js:resolveSwipeAction(dx,dy,anyDrawerOpen,threshold=60)`（**`Number.isFinite` fail-closed** + 主轴 `ax>ay` 严格；开着抽屉任一明确水平滑→`close`，没开→右滑 `openLeft`/左滑 `openRight`）；`MobileShell` 根 div 绑 `onTouchStart/onTouchEnd/onTouchCancel`——**`touchstart` 用 `changedTouches[0]` 记本次新增手指 + 按 `identifier` 配对、`touchend` 找不到同指就 `return`（不 fallback `list[0]`，防多指起终点错配）**；`touchstart` 落在 `input/textarea/select/button/a/[contenteditable]/.overflow-x-auto/.fixed` 上**不接管手势**（防选字/横滚/点按误触发；`.fixed` 2026-07-12 加——弹窗多开在抽屉子树内，滑关抽屉会连带 `visibility:hidden` 掉正开着的弹窗）；**不 `preventDefault`**（保纵向滚动）、**不做 follow-finger / 零 transform**（只改 drawer state，靠现有 off-canvas transition 动）。Codex 双轨 3 轮（fail-closed / target-filter / 多指错配）APPROVED。已知边界：贴屏幕**最边缘**起手会被手机浏览器抢去当后退/前进 → 开抽屉从中间滑最灵、按钮兜底。回归 `tests/drawerSwipe.test.mjs` + `mobileShell.source.test.mjs`。
- **移动端「进项目后整个假死」修复（2026-07-12，试用反馈「选项目后左右滑没反应 + 上下滑不了 + 上滑变刷新」，Codex 3 轮 APPROVED + 部署 kr-web-01 bundle `index-Pe1F-E-O.js`）**：**主因＝flex-col 滚动陷阱**——MobileShell 用 `flex flex-col` 包 ChatPanel，而 ChatPanel 根曾缺 `min-h-0`（flex 子项主轴默认 `min-height:auto` 不小于内容高）→ 长对话把根撑破视口（真浏览器复现：600px 壳里撑到 3500px）→ 消息区 `overflow-y-auto` 永不可滚 → `messagesEndRef.scrollIntoView()` 转而去滚 **overflow-hidden 的壳根**（scrollTop 被顶到 2900）→ 顶栏/抽屉/scrim 全滚出屏幕，表现为整个 UI 假死（滑不动、抽屉「开了但在屏幕外」、无处可滚时链到文档触发下拉刷新）。**修＝ChatPanel 根加 `min-h-0`**（`flex-1 min-w-0 min-h-0 bg-chat flex flex-col`，源码守护锁死**勿删**）；桌面是 row 布局（高度走 stretch）零影响。诊断线索：空项目一切正常、有内容才炸 = 先查 min-height 链，别先怀疑手势。同批**防御性加固**（都是触摸设备 scoped、桌面零影响，非主因但保留）：① 壳根 **`touchAction: 'pan-y pinch-zoom'`**（横向 pan 不交浏览器接管——接管发 `touchcancel` 吞手势；内层 `.overflow-x-auto` 横滚不受影响——手势链止于最近滚动容器；**绝不能加 `pan-x`**）；② `index.css` **`@media (pointer: coarse){html,body{overscroll-behavior:none}}`**（杀下拉刷新 + 水平历史导航；`overscroll-behavior` 只许出现在该块内，守护锁死）；③ coarse 块内表单控件 `font-size:max(1em,16px)!important`（iOS 聚焦 <16px 输入框自动放大整页；`!important` 压 Tailwind 字号类特异性）；④ viewport meta 加 `interactive-widget=resizes-content`（安卓 Chrome 108+ 键盘收缩布局视口，iOS/桌面忽略）；⑤ 壳根 `h-screen` 类作 100dvh 解析兜底。**两条铁律（Codex 首轮 BLOCKER，勿改回）**：`onShellTouchCancel` 必须保持**纯清理**（cancel 也代表系统打断，拿最后坐标补判会误开关抽屉）；**`viewport-fit=cover` 禁止加**，除非全站做齐四边 `env(safe-area-inset-*)`（半吊子 cover 会让顶栏/抽屉/admin 页钻进刘海安全区）。已知支持底线：iOS 16 以下无 `overscroll-behavior`。回归 `tests/mobileViewport.source.test.mjs`（新，含 ChatPanel min-h-0 锁）+ `mobileShell.source.test.mjs` 扩充。
...
## docx 导出排版模板（2026-07-16 实施 + Codex 2 轮审 APPROVED（首轮 4 BLOCKER 已修）+ 部署 kr-web-01 第十二笔；排版已过用户 Word/WPS 实机验收）
...
- **回归**：`tests/test_report_tools.py`（预处理对抗/中和/净化/temp 清理/真 pandoc E2E 含恶意域断言）+ `test_packaging_spec.py::DocxTemplatePackagingTests`；模板完整性测试锁 CRC/全 XML well-formed/rels 断链。排版已过用户 Word/WPS 实机验收（2026-07-16，「格式效果都不错」）；**待办**：Windows 打包烟测；07-17 目录修复版待用户复验（见下「### 目录固化」段）。

### 目录固化（2026-07-16 首版 → **2026-07-17 架构重做：LO=页码 oracle**，Codex 审 APPROVED + 部署 kr-web-01）

背景：WPS 不认 `updateFields`、不会打开时自动更新 TOC 域（固有行为）→ 需要导出时把目录固化成静态条目+页码。**首版（07-16）让 LibreOffice `storeToURL` 重新导出整份 docx，用户实测暴露两硬伤后已废弃**：LO 的 docx 导出回写产生未闭合/重名书签 + `__RefHeading__` 私有锚点（WPS 点目录条目报「无法打开指定的文件」）、在首个正文标题段塞显式 `<w:br w:type="page"/>` 与样式 pageBreakBefore 双重分页（目录后空白页 + 目录页码整体偏 1）、还丢 settings updateFields。**绝不回退到「LO 导出文档」路线**。改导出固化 / LibreOffice 子进程前必读：

- **现行架构（07-17）**：`lo_fixate.py` 只当**页码 oracle**——UNO 加载 docx、内存里 `update()` 目录索引、把「条目文本+页码」写成 JSON（`{"entries":[{"text","page"}]}`，argv=`[SOFFICE_BIN, IN.docx, OUT.json, PROFILE_DIR]`），**绝不 storeToURL**；`report_tools._fixate_toc_fields` 在**原始 docx** 上做字符串手术：正文 Heading2-4 段注入 `_Toc9XXXXXXXX` 书签（id 避让既有书签 max+1）+ 占位 TOC 域段替换为 **Word 原生形态**静态条目（`<w:hyperlink w:anchor w:history="1">` + 文本 + tab + PAGEREF 域缓存页码；H2→TOC1/H3→TOC2/H4→TOC3，与 Word F9 同映射；域壳 begin/instr/separate…end 跨段保留、指令仍是 ` TOC \o "2-4" \h \z \u `）。文档其它 part 字节级不动 → 图表/页脚 PAGE 动态域/updateFields 天然保留；Word 打开仍整体重算（w:dirty+updateFields），WPS 显示静态值、F9 也能正确刷新（PAGEREF+书签都是真的）。
- **对账 fail-closed**：docx 标题清单（`_scan_docx_headings`，只扫 H2-4；段边界/形状不符返 None）与 oracle 条目按**数量+逐条文本**（`_toc_match_key` 去空白）对账，任何不匹配/坏 JSON/页码越界（非 int 或 <1/>9999）→ 降级保留 TOC 域。正文混入违规 H1 时 LO 索引会多收条目 → 数量对不上 → 自动降级（刻意行为）。**目录显示文本用 docx 标题原文**（oracle 只供页码，LO 文本差异不外漂，codex NIT1）。手术产物必须过 `ET.fromstring` 才写回（temp+`os.replace` 原子，只换 `word/document.xml`）。
- **进程/降级硬约束（沿用首版 codex 4 BLOCKER，勿回退）**：① helper `start_new_session=True` 自成进程组，超时 `_terminate_process_group`（`os.killpg(proc.pid, SIGKILL)`，PGID==PID + 有界 reap）连带杀 soffice 孙进程；PROFILE_DIR+oracle JSON 都在 `mkdtemp(prefix="cra-fixate-")` work 目录、`finally` 无条件 rmtree（含硬杀路径）。② 整个 `_fixate_toc_fields` 外层 `try/except Exception→False`，**绝不抛**、绝不影响导出成功。③ soffice 路径 argv 透传不硬编码；`lo_fixate.py` 不 import backend；`cwd=PROFILE_DIR`（soffice 启动脚本 cd 回启动 CWD，避开不可访问 CWD）。④ `_resolve_uno_python` 锁 + 只正缓存；缺 LibreOffice/uno 去重 warning。
- **平台边界**：仅 web/服务器态生效；打包/Windows 桌面态首行 `sys.frozen or win32` 直接返 False（保留 TOC 域，Word 自动更新、WPS 手动）；`lo_fixate.py` 无需进 PyInstaller datas。**可调**：`CRA_LO_PYTHON`、`_LO_FIXATE_TIMEOUT_SECONDS=90`。
- **回归**：`tests/test_report_tools.py::TocFixationTests`（跳过条件/成功形态含书签体系唯一+闭合断言/重名标题按序配对/数量·文本·页码对账降级/坏 JSON/ZIP 写回失败保原/非 document part 字节级不动/超时 killpg/绝不抛/正缓存）+ 导出文案两分支。服务器 e2e：18 项结构断言 + LO 转 PDF 渲染级校验（7 页无空白页、目录页码与各章实际落页逐条一致）。

## 试用反馈批次 0710（2026-07-11 实施 + Codex 单轨审 2 轮 APPROVED（首轮 2 BLOCKER 已修）+ 部署 kr-web-01 第十笔 + 真模型 GUI E2E 全过 + push origin）

陈燕（0710 表序号 5）反馈四件套：流程图布局修复 / 管理办法颗粒度化 / 自定义搜索 key / 初次引导。spec `docs/superpowers/specs/2026-07-11-mgmt-doc-granularity-and-flowchart-layout-design.md`（§11 实施期修订以本节为准）。改 flowchart 布局 / 文档参数槽 / 搜索路由 / 引导前必读：

- **流程图（`diagram_render.py:_flow_layout` 纯布局 + `_render_flowchart` 消费几何）**：近线性多层流（`n_layers >= FLOW_VERTICAL_MIN_LAYERS(5) 且 max_rows <= FLOW_VERTICAL_MAX_ROWS(2)`）走纵向一层一行；两朝向都「字随框走」（断行宽度从实际框宽反推，`_flow_wrap_lines` 以 CJK 全宽为保守单位）；装不下→`ChartRenderError` 友好失败（纵向 >`FLOW_MAX_VERTICAL_LAYERS(12)` 层 / 横向过密 / 标签 >3 行），**失败文案不得建议 process 模板**（其上限 8 步装不下长流程）。⚠️ **12 层 × `FLOW_MIN_ROW_HEIGHT_IN(0.5)` 恰满 6.0in axes 预算，纵向任何行加高都会顶爆**（钻石行因此严格画在分配框内、不加高）。几何测试直连 `_flow_layout`（overlap/bounds/断行容量断言），不做字节级 PNG 快照。其余 19 图源码零改。**PNG 不可变 → 修复上线后存量糊图不自愈，需重新生成换引用**。
- **管理办法颗粒度（`skill.py` + `chat.py` + 模板）**：`project-overview.md`「## 文档参数」槽（模板**自带填写说明**——kickoff trigger_prompt 是 transient system message 只注入开场轮，指令要活过开场只能靠随 `CORE_CONTEXT_FILES` 每轮注入的模板文字；取值行除取值不得有其它文字，否则全等解析不中）；`parse_management_doc_params` **双语别名闭集严格全等**（`MANAGEMENT_DOC_GRANULARITY_ALIASES`/`_CLAUSE_ALIASES`，禁子串/casefold/NFKC；**只扫「## 文档参数」段内**；缺失/占位符静默默认、有值不合法→默认+固定文案 advisory **绝不回显槽内容**）；`load_type_skeleton(project_type, granularity="standard")` 默认参数其余 6 类+standard 逐字零回归（守护测试 legacy 抽取对比）；**条款样式指令必须 S1–S4 全阶段注入**（挂 `build_methodology_block` 的 instr 尾部、按 `project_type=="management-document" && clause_format is not None` 门控——只挂 S1 declare 分支是 spec 原稿 bug，S4 写条款时会缺席）；`_populate_v2_plan_files` 对非 management-document 剥槽。开场白 builder `_build_system_trigger_prompt`：未知 trigger 返 None 且**不查项目**；非 management-document 逐字等于 `SYSTEM_TRIGGER_PROMPTS` 静态文案。
- **自定义搜索（config/url_guard/search_providers/chat/main/SettingsModal）**：per-uid 三字段**与模型 mode 完全独立**（managed 也可配，不强制成对出现——用户 2026-07-11 拍板）；配置 provider+key → `_web_search` 最前分叉走 `_custom_search`：绕过池子路由与**全部限额门禁**（单轮/分钟/全局；max_iterations 天然兜底）、**不入池子记账**；结果装配复用 `search_pool.build_provider_success_payload`（与池子契约一致）。**SSRF 三道**：`validate_custom_search_api_base`（https+无 userinfo+解析公网，**无域名白名单**——用户拍板）保存时校验 + `_custom_search` 逐调用复验 + **自定义实例必须 `follow_redirects=False` + `session.trust_env=False`**（Codex 红队 BLOCKER：requests 默认跟随 302，恶意中转可重定向到 metadata/内网；池子官方端点保持默认不变）。适配器 `api_base` 覆盖：无路径拼默认 path、带路径原样用、实例属性不污染类。前端掩码 key 三规则（Codex BLOCKER）：切渠道清 key 强制重填、切回载入渠道恢复掩码、停用（渠道空）提交 `'***'` 保留已存 key 不拦不清。`per_turn_searches` 5→10 是 `managed_search_pool.json` 配置——**部署时服务器副本同改+重启**（路由单例不热重载）。
- **初次引导（accounts/main/OnboardingTour/App）**：`users.onboarded_at`（`init_db` 幂等 ALTER）「终身一次」真值在服务端；`/me` 透 `onboarded`（桌面 local 合成 true）；`POST /api/auth/onboarded` 幂等（不覆盖首次时间）。前端 App 门控**严格 `authUser.onboarded === false`**（老会话无字段→undefined 不弹，fail-closed）；渲染点在 isMobile 三元分支**之外**（fixed 覆盖层两壳通用）；onDone `{...prev, onboarded:true}` **只翻一个字段**（init effect 依赖 `[uid, must_change_password]` 雷区不动）；回写失败也 `finally: onDone()`（绝不把用户困在弹层）。
- **回归**：`tests/test_chart_render.py::FlowchartLayoutTests`（锚样例纵向/无重叠/友好失败三态）、`test_skill_engine.py::ManagementDocGranularityTests`（解析对抗/骨架/S1-S4 注入/逐字守护）、`test_chat_runtime.py`（builder 分叉/`CustomSearchKeyTests`/`CustomSearchSsrfHardeningTests`）、`test_url_guard.py::CustomSearchApiBaseTests`、`test_settings_api.py::CustomSearchSettingsTests`、`test_search_providers.py::ApiBaseOverrideTests`、`test_accounts.py::OnboardedColumnTests`、`test_auth_api.py::OnboardingTests`、前端 `settingsModal.source`/`onboardingTour.source`。后端 1774 / 前端 582 / build 全绿。**明确不做**：陈燕 #1 redline、#5 导出格式预设、#7 引用防编造、#8 卡死（无证据）——台账见 spec 附录 B。**部署（第十笔，2026-07-11）**：12 文件 file-push + dist swap `index-BbWuMF6t.js` + 服务器 `managed_search_pool.json` per_turn 10（⚠️ gitignored，两侧手改）+ 重启（onboarded_at 迁移自动）；回滚点 `/opt/cra-rollback-20260711/`。真模型 GUI E2E 全过（test 账号，凭据在 VPS note）：引导终身一次闭环 / 开场真问三参数 / 槽位写出精确枚举 / 设置页停用不困；仍挂 S4 真模型图表全链（worklist）。

## 试用反馈批次（2026-07-09 实施 + Codex 单轮审+3 轮红队 APPROVED + merge main `c5e2ca4`（--no-ff）+ 部署 kr-web-01；分支 `fix/trial-feedback-0709` 保留）

试用反馈 0709 四件套（文件栏乱跳 / 义务机制手术 / 新建项目自动需求确认 / 文件内链）。**部署（第七笔，2026-07-09）**：dist swap bundle `index-DnpSREU1.js` + file-push 3 后端文件（chat/models/report_writing，sha256 校验一致）+ systemd 重启；公网 smoke 过（health 200 / 新 bundle / `/admin` 200 / 鉴权 401）；回滚点 = 服务器 `/opt/cra-rollback-20260709/`（3 旧后端文件）+ `frontend/dist.old`。改文件树刷新 / 轮末对账 / system_trigger / 聊天区文件链接前必读：
...
**拓扑**：CRA → 薄网关 `managed_proxy`（纯字节透传、不碰 usage）→ new-api（渠道路由 + 计费/日志）→ **本 sidecar** → opencode。

**`x-opencode-session`（2026-09-18 加）**：opencode Go 自 2026-09-06 起缺此头直接 400 `MissingSessionID`（new-api 不对 400 failover → 渠道 61 全挂 12 天，CRA 试用通道、data-news、hermes 同时中招）。new-api 不透传客户端自定义头，故 sidecar 统一补：入站已带则沿用，否则 `ses_`+sha256(model+首条 user 消息)[:32]（刻意不含 system——其尾部项目状态逐轮变），非 chat 请求随机 uuid；UA 设 `cra-opencode-normalizer/1.1`。网关再出 400 先查此头。缓存字段全程透传到 CRA（2026-07-03 门禁 + 薄网关全链实测：8/8 响应带 `prompt_cache_hit_tokens>0`，含走渠道 61 的）。
