...
最后更新：2026-06-26（**前端 UX 翻新实施完成 + 每批及整分支 Codex 双轨审全 APPROVED，待用户本地终验 → merge/部署**——分支 `feat/frontend-redesign`，39 commits ahead of main，**未 merge / 未 push / 未部署**）：全部 8 批次（0 地基 → 1 App 外壳/主题 → 2 Login/ForcePasswordChange → 3 Sidebar → 4 MarkdownMessage/ThinkingBlock/ChatPanel → 5 WorkspacePanel/StagePanel/StageAdvanceControl/FilePreviewPanel/RollbackMenu/ConfirmDialog → 6 ProjectCreateModal/SettingsModal/AdminPanel/IndependentReviewDrawer → 7 整分支验收）换皮完成。**通道 token 单一真值源**（`index.css` `:root/.dark` + `tailwind.config.js` 语义类 + `utils/theme.js` + `index.html` 防闪 bootstrap + `components/icons.jsx` 线性 SVG + 自托管 woff2 Hanken/IBM Plex）；FilePreviewPanel 代码高亮改 token 双主题、KaTeX 随主题。**护栏**：`paletteGuard`(ALLOW_PENDING **已清空**=全量迁移)/`tokenContract`/`darkClassGuard`(仅许 `dark:bg-scrim/N`)/`themeBootstrap`/`theme.test` 全绿；前端 **414 测试全绿 + `npm run build` 绿**。**追加（用户本轮要求，非原 plan，已记 plan「## 实施期范围增补」）**：① 左侧栏加宽 248→264px + 可整列收起/展开（开关在聊天 header 左侧、与右侧工作区开关镜像图标 IconSidebar↔IconPanelRight、showSidebar 持久化 + 重夹拖宽）；② **材料 tab 直接上传**（复用聊天回形针同一 `/materials/upload` 端点，上传即入库；忙态按 projectId 作用域；project-switch + unmount + StrictMode 守卫齐全）。**Codex 双轨审（用户重登后补齐，全 APPROVED）**：批次 0–5 backfill（spec 1 轮 / quality **4 轮**——挖出并修一串上传守卫真 bug 含 unmount/StrictMode 静默失效、stepper report-only S7 116% 溢出、S2/S3 进度条误标「正文字数」、材料 tab 工作目录展示行被换皮漏掉=功能回退、删除按钮丢 aria-label）+ 批次 6（spec 1 轮——修 IndependentReviewDrawer 把 422 数组 detail 直接进渲染态的崩溃隐患，抽 `normalizeApiErrorDetail` 共享归一；quality 1 轮 clean）+ **整分支双轨终审 APPROVED**（仅 NIT：prose-invert 浅主题反色风险/auth 卡片任意 shadow→shadow-popover/bootstrap catch 兜底去 .dark/上传忙态项目作用域，已全修）。每批 commit 后即审（项目约定，[[per-batch-codex-review]]）。本地隔离实例 `run_web.py`（`CRA_DATA_ROOT=<scratchpad>/cra-data`，admin/邀请码自配）`localhost:8888` 已起、serves 最新 build。**下一步**：用户在本地实例做最终双主题逐屏验收（**刷新取新 bundle**；重点看批次 6 四弹窗：新建项目/连接设置/用户管理/独立审查）→ 确认后 `git merge feat/frontend-redesign` → 作为新版部署 kr-web-01（前端 only 部署流程见项目 CLAUDE.md「中右分栏拖动…」段末「部署（前端 only…）」）。见 memory [[frontend-redesign-status]]。

**前端 follow-up（用户 2026-06-26 本地测试提出，待单独 plan + 实施 + Codex 审）：工具调用卡片重设计 + 去 emoji**。现状：助手消息里的工具调用渲染成多行原始日志卡片（`🔧 调用工具: edit_file({...完整 JSON...})` + `✅ 结果: {完整 dict}` + `准备调用工具: edit_file`，且带 ✏️/🛠️/✅ 等 emoji——emoji 来自后端 `chat.py` 发的 SSE tool 事件文本，**不在前端源码里故 paletteGuard 扫不到**）。目标：对齐设计原型 `design_handoff_frontend_redesign/Prototype-standalone.html` 的优雅单行 pill（`<>` 线性图标 + 粗体 mono 工具名 + 灰字主参数如文件路径 + 末尾 ✓/⚠ 线性图标），**保持当前信息量**（工具名 + 操作对象 + 成功/失败 + 简短 message），去掉所有 emoji。**改动面（中等、纯前端可行但非琐碎）**：① 新解析器把 tool 日志行解析成结构化 `{kind:call|result, tool, primaryArg, ok, message}`（注意 args 在显示里已被截断成非法 JSON `…)`，primaryArg 要按 `"file_path"|"path"|"query"|"url"` 等启发式提取）；② 把 call 行与紧随的 result 行**配对**成单个 pill；③ `splitAssistantMessageBlocks` 的 tool-line 检测靠 emoji 前缀（`🔧 调用工具:`/`✅ 结果:`/`⚠️ 结果:`）——改渲染时别破坏检测（或迁移检测逻辑）；④ 「准备调用工具」行现渲染成纯文本（未被检测），决定隐藏或并入 pill；⑤ 同步 `stripToolLogComments`/`getCopyableAssistantMessageText`（复制逻辑）+ `chatPresentation` 测试。可选后端：把 SSE tool 事件改成结构化（去 emoji、给 toolName/arg/status 字段）更干净，但扩到后端 + DeepSeek 兼容面，**优先纯前端解析现有文本**。先写 plan 再做。
