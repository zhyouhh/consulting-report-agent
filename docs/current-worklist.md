...
**W2. 服务器化 + 多用户 Web 化** — 状态：`B1 ✅ merge（c62cd4d）+push；B2 ✅ merge（c2916b1）+push origin；B3 ✅ merge（450acba，--no-ff）+push origin`——**W2-B 三段（B1 租户基座 / B2 中央计费 / B3 admin+安全硬化+custom 激活）全部实施完成并合并 main。** 6 Phase subagent-driven + 每 Phase Codex spec/quality 双轨独立 review（含红队对抗）+ 收尾全分支综合审 SHIP-READY。后续 backlog（非 W2-B 范围）：原子 reserve / 多 worker 共享 `_LOGIN_FAILS`·`_RUNTIME_ALLOWED_HOSTS`·`_miss_counter` / pinned-IP-with-SNI transport（彻底防 DNS rebinding）/ CAPTCHA·MFA（彻底解登录限流三难）/ 原生 fetch SSE 401 统一跳登录。
