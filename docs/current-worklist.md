...
**W2. 服务器化 + 多用户 Web 化** — 状态：`B1 ✅ merge（c62cd4d）+push；B2 ✅ merge（c2916b1）+push origin；B3 实施中（分支 feat/w2b-b3-admin-security-hardening，未 merge/未 push）：Phase 1（SSRF+custom 激活）/Phase 2（CSRF·CORS·限流）/Phase 3（admin 函数+API）已落地并 Codex 双轨 APPROVED；Phase 4（must_change_password 路由级强制，Task 14）✅ 完成——commits 69cafda+99a53e0、Codex 红队双轮独立复审 APPROVED 零 BLOCKER（仅 2 非阻塞 NIT）、关键回归门全绿、显式 8 端点+require_project+get_current_admin 串依赖、豁免集精确 4 项（me/change-password/logout/health）；下一步 Phase 5（前端 admin 面板 + 强制改密屏）`
...
- **spec**：`docs/superpowers/specs/2026-06-21-w2b-multi-tenant-core-design.md`（4 轮 APPROVED）。**实施拆 3 plan**：**B1** 租户基座+鉴权+创建闭环（`plans/2026-06-21-w2b-b1-tenant-base-auth.md`，18 TDD task，5 轮含 2 轮红队 APPROVED）**✅ 已实施落地**（subagent-driven + 每 commit Codex 双轨独立 review + 全分支综合审 SHIP-READY；cutover `docs/superpowers/cutover_report_2026-06-22_w2b-b1.md`；后端 1272 passed/4 mac-realpath、前端 334、真实 HTTP smoke + chrome GUI E2E 全过；merge `c62cd4d` + push origin；分支保留）/ **B2** 中央计费 `MeteredManagedClient`+per-user ¥配额（**✅ 实施完成 + merge `c2916b1` + push origin**）/ **B3** admin 面板 + CSRF/SSRF/CORS 硬化 + custom 激活 + must_change_password 强制 + per-username 限流（**✅ 实施完成（待 merge，等用户）**——分支 `feat/w2b-b3-admin-security-hardening`，5 Phase subagent-driven + 每 Phase Codex 双轨独立 review 全 APPROVED + 红队对抗；新叶子 `backend/url_guard.py`（三层域名白名单 SSRF）+ CSRF 中间件 + throttle-first 登录限流 + 8 admin 端点（`admin_set_user_disabled` BEGIN IMMEDIATE 原子守卫）+ `require_password_current` 三层覆盖 + custom 真激活（`mode` 现持久化）；后端 1419 passed/4 mac-realpath、前端 357、build 绿、DeepSeek 兼容+跨租户隔离绿；cutover `docs/superpowers/cutover_report_2026-06-23_w2b-b3.md`）。**W2 状态：B1✅merge / B2✅merge / B3✅实施完成（待 merge，等用户）。**
