...
**前端 follow-up：工具调用卡片重设计 + 去 emoji —— ✅ plan 已写（待 round-2 Codex 复审 + 实施）**：`docs/superpowers/plans/2026-06-26-tool-call-pill-redesign.md`。**方案**（红队修订后定稿）：后端把**正常**工具 call/result 的 emoji 文本 SSE 换成结构化 `tool_call`/`tool_result`（带 `id` 配对，独立审查也发 `id`），前端 content 内塞 sentinel `<<tool-call:ID>>` 保「文字-工具-文字」行内顺序 + `message.toolEvents` map（`reduceToolEvent` by id）+ 共享 `ToolCallPill` 组件（IconTool + 工具名 mono + 参数淡 + 成功 IconCheck/失败 IconClose/进行中脉冲点），主聊天与独立审查统一渲染。**铁律**：只改 SSE 呈现层 + 前端，**不碰 DeepSeek provider 序列化、不碰 tool-log 持久化 `_format_tool_pair_line`**；**保留** chat.py 诊断类 `type:"tool"` 文本事件（禁工具/畸形/漏写重试，ChatPanel legacy 分支不删）。Codex round-1 对抗红队挖 **6 BLOCKER**（① 初稿会误删诊断 `type:"tool"` 消费方；② `_format_tool_pair_line` 重构破持久化且测试 `assertIn` 锁不住；③ 结构化事件测试放错文件[应 `test_chat_runtime.py` 非 `test_stream_api.py`]；④ 漏更 `test_chat_runtime.py` 旧 emoji 断言；⑤ 去 emoji grep 会误伤诊断 ⚠️；⑥ pill 注释含 ✓/✗ 撞 paletteGuard）已全修订进 plan；**round-2 复审撞 Codex 额度上限（6:19 重置）待补**。**非目标**：reload 后 pill 持久显示（当前即不显示、无回归；需改持久化为结构化 JSON 单列 follow-up）。现状（背景）：工具调用渲染成两 pill/工具的 emoji 日志（`🔧 调用工具`/`✅ 结果`，写死绿勾、emoji 来自后端 SSE 文本故 paletteGuard 扫不到）。
