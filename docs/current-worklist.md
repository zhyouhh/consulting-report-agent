...
最后更新：2026-06-26（**前端 UX 翻新执行中：批次 0–5 已完成并 commit + 侧栏增强；批次 6–7 + Codex 双轨审待续**——分支 `feat/frontend-redesign`，subagent-driven 逐 task 实施，**未 merge**）：已迁移并 commit——地基（通道 token `index.css` + `tailwind.config.js` + `utils/theme.js` + `index.html` 防闪 bootstrap + `components/icons.jsx` + 自托管 woff2 字体）、App 外壳 + 主题接线、Login/ForcePasswordChange、Sidebar（账户区重组 + 主题切换 + 删除弹窗）、MarkdownMessage/ThinkingBlock/ChatPanel（按区域换皮、保住流式/credentials/SSE/乐观清空/停止/附件全套不变式）、ConfirmDialog/StageAdvanceControl/RollbackMenu/StagePanel（stepper 7/8 段由 delivery_mode）、FilePreviewPanel（**代码高亮改 token 驱动双主题** `:root/.dark` hljs 变量、删 github-dark.css 写死、KaTeX 随主题）、WorkspacePanel（三 tab 段控 + 材料库）。**追加（用户本轮要求，非原 plan）**：左侧栏加宽 248→264px（连接卡型号不再换行）+ **左侧栏可整列收起/展开**（开关放聊天区 header 左侧、与右侧"切换工作区"左右**镜像图标** IconSidebar↔IconPanelRight；showSidebar 持久化 + 显隐重夹拖宽 clamp）。**护栏**：`paletteGuard`/`tokenContract`/`darkClassGuard`/`themeBootstrap`/`theme.test` 全建；paletteGuard `ALLOW_PENDING` 只剩批次 6 的 4 个（AdminPanel/IndependentReviewDrawer/ProjectCreateModal/SettingsModal）。前端 **408 测试全绿 + `npm run build` 绿**（每批后跑），chrome-devtools 双主题逐屏冒烟（登录/改密/主界面/工作区浅深）已过、字体硬门验证自托管 Hanken/IBM Plex 真 loaded。本地隔离实例 `run_web.py`（`CRA_DATA_ROOT=<scratchpad>/cra-data`，admin/邀请码自配）已起供测试。**⚠️ 流程纠正（用户 2026-06-26 当场纠正）**：本应**每批 commit 后走 Codex spec+quality 双轨审**（项目约定），本轮把 Codex 审推后了、且赶上 codex token 过期（refresh token 失效，需 `codex login` 重登）未跑成——**待用户重登 codex 后：补齐批次 0–5 的双轨审到 APPROVED，再逐批审批次 6**。见 memory [[per-batch-codex-review]] + [[frontend-redesign-status]]。**下一步**：批次 6（ProjectCreateModal/SettingsModal/AdminPanel/IndependentReviewDrawer 换皮，保住 web payload 不发 workspace_dir / managed_base_url 只读 / AdminPanel 可编辑额度 input / 审查浮窗 run_id+状态机+credentials+detail 归一）→ 批次 7（ALLOW_PENDING 清空 + 全量测试/build + 按角色对比度 + 双平台逐屏对照 + Codex 整分支双轨审）→ 本地实例终验 → 用户确认后作为新版部署 kr-web-01。
