...
| ~~`backend/chat.py` full-draft rewrite branch lines 5547-5584~~ | (含在 `_validate_required_report_draft_prewrite` 5531-5615 整段删除内) | — | **删** — 之前 v3 误标"保留"；与上面整段删除条目重叠，避免歧义统一为删，因为新 `rewrite_report_draft` 工具入口 inline 复刻 full-draft 校验逻辑（per §2.4 + §7.4 + §10 Q5 inline-migrate 决议）|
