...
替换当前的 `<draft-action>` tag + keyword fallback + scope enforcement 整套机制（fix4 v5 amendment 的核心）成 **3 个专用写正文工具**。模型通过工具调用本身声明意图，**后端用 preflight 已 resolve 的 target snapshot 自己控制 `old_string`**——彻底消除 model 必须精确复述 1500 字章节原文这个失败模式。

净简化：删 ~2000 行后端代码 + 800 行测试，加 ~200 行工具实现 + 400 行测试 + 120 行 spec 文档。
...
**新增**：
- `user_message_text`（**new turn_context field**）— 在 `_build_turn_context` 中用现有 helper `_extract_user_message_text(current_user_message)` 取出 raw user msg 字符串并 cache。新工具在 `rewrite_report_section` 入口读这个字段做 `resolve_section_target` 解析。**理由**：避免每个工具自己再 traverse current_user_message 结构，保持一致；另也让 mocking / unit tests 容易注入。

**保留**：
- `read_file_paths`（用于 read-before-write check）
- `web_search_performed` / `fetch_url_performed`（用于 fetch_url gate）
- `pending_system_notices`（用于 system_notice 去重）
- `pending_stage_keyword`（stage-ack 系统）
- `can_write_non_plan` / `generic_non_plan_write_allowed`（保留 — `_should_allow_non_plan_write` 路径暂不动）
- `checkpoint_event`（stage-ack 系统）

**删除**：
- `canonical_draft_decision`
- `required_write_snapshots`（不再需要"轮内对账"，因为新工具一次调用就完成写入；如下游 `_required_writes_satisfied` 等代码路径有依赖，需要 spec reviewer 评估是否一并清理或留作"通用 plan write 对账"）
- `draft_action_events`
- `compare_baseline_event_count`
- `canonical_draft_mutation`（如果当前在用）
...
| `backend/chat.py` | `_validate_required_report_draft_prewrite` 中专门 for canonical draft 的章节 / replace_text 分支 | -120 | **删/简化**（改为新工具的 inline check） |
...
| `backend/chat.py` | `REPORT_BODY_*_KEYWORDS` 系列常量 | -40 | **删**（如 `REPORT_BODY_FIRST_DRAFT_KEYWORDS`、`REPORT_BODY_EXPLICIT_CONTINUATION_KEYWORDS` 等） — 实施前 grep 各常量的全部引用，仅在 `_classify_canonical_draft_turn` body 内引用的可随之删；其他引用（如 `_should_allow_non_plan_write`）需保留 |
| `backend/chat.py` | `REPORT_BODY_REPLACE_TEXT_INTENT_RE`, `REPORT_BODY_INLINE_EDIT_RE` | 0 | **保留** — `replace_report_text` 工具内部不用（model 直接给 old/new 参数），但 `_should_allow_non_plan_write` 等通用门禁有依赖；不删 |
| `backend/chat.py` | turn_context 新增 `user_message_text` 字段（在 `_build_turn_context` cache）+ `_new_turn_context` 添加默认 `""` | +5 | **新增** |
| `backend/chat.py` | 3 个新工具的 callable + tool dispatch 注册（约每工具 50-80 行） | +200 | **新增** |
| `backend/report_writing.py` | 新模块（共享 helpers + `resolve_section_target` 迁移自 fix4 fix2） | +180 | **新增** |
...
|---|---|---|---|
| `tests/test_draft_action.py` | 整个文件 | -250 | **删** |
| `tests/test_chat_runtime.py` | `DraftActionParserTests` | -200 | **删** |
| `tests/test_chat_runtime.py` | `StreamSplitSafeTailDraftActionTests` | -80 | 改剪（保留 stage-ack 部分） |
| `tests/test_chat_runtime.py` | `PreflightCheckTests` | -150 | 大部分删，stage gate / outline check 部分迁移到新工具测试 |
| `tests/test_chat_runtime.py` | `DraftActionPreCheckTests` | -100 | **删** |
| `tests/test_chat_runtime.py` | `GateCanonicalDraftToolCallTests` | -300 | **删** |
...
| **测试净删** | | **~-1280 行**（再加新工具 unit + integration tests ~+500，净 -780） | |
...
1. **Unit tests pass**: 新工具 `rewrite_report_section` / `replace_report_text` 的所有正/反 case 全 pass；`backend/report_writing.py` helpers 单测 pass；`tests/test_chat_runtime.py` 删除目标 class 后剩余 test 全 pass
2. **Wider sanity**: chat_runtime 全 suite 0 fail（仅整个删除的 class 不再存在）
3. **Frontend tests**: 168 / 168 pass（不动 frontend）
4. **Build**: `build.ps1` 重新打包成功，dist/ 大小不超过当前 ±5%
5. **Spec / SKILL.md**: §4.3-§4.12 标注 superseded；SKILL.md §S4 替换为新表格；本 spec doc 进 main
6. **Cutover smoke**: 4 sessions（A 起草 / B 重写第二章 / C 替换文本 / D 续写第三章）至少 3 个写盘成功（含 B 章节实际被替换为模型 content）；剩 1 个允许 model behavior issue 但工具不卡 dead-loop
7. **No regression**: stage-ack 系统所有现有 test pass；`<stage-ack>` tag 在 cutover 期间正常工作
...
1. **方案 X vs Y 选择**（§6）— spec reviewer 拍板
2. **§5.1 中 `REPORT_BODY_REPLACE_TEXT_INTENT_RE` 是否保留**：当前 `_should_allow_non_plan_write` 等可能依赖；迁移前需 grep 全引用列表
3. **`_validate_required_report_draft_prewrite` 保留的具体边界**（§7.4）— 确认逻辑切片精确
4. **是否在 v1 加 content 总长度上限**（§7.1）— 简单 cap 值得 vs YAGNI
5. **`backend/report_writing.py` 模块拆分**（§3.1）— 是新模块还是塞进 `backend/chat.py`？模块化更清晰，但项目约定通常往 chat.py 加；reviewer 给意见
