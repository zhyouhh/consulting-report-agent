...
waiter、上传/删除 token、桌面/移动统一接线）；后端 SSE 线程池扩到 16；DELETE 采用
request→review 锁序、5 秒超时 409 与锁后项目复验；registry、`materials.json`、conversion
cache ref sidecar 均已做锁内原子 RMW。工具参数支持 schema 内 camelCase→snake_case 容错；
同步/流式 DSML 泄漏自愈并覆盖持久化与历史输出净化；`[DL-...]` 从写作 prompt、独立审查到
docx 导出三层收口；四类研究报告只在 S4 注入研究写作纪律。

**质量门**：后端全量 `1857 passed, 1 skipped, 4 deselected, 176 subtests passed`；前端全量
`606 passed`；生产 build 通过；独立红队在修复 ref 重绑丢 waiter、删除失败 stale prop 恢复、
forget ghost work 三处真实竞态后给出 `APPROVE`。

**部署**：已原子部署 `kr-web-01`（新 bundle `index-DlBZZnSg.js`，systemd 单 worker）；远端
运行文件 SHA-256 与本地一致，service/journal 正常，公网 health 200、SPA shell no-cache、
hash asset immutable、未登录鉴权 401、真实浏览器登录页零 console error。回滚点：
`/opt/cra-rollback-20260718-105702` + 服务器 `frontend/dist.old`。详见
`docs/superpowers/cutover_report_2026-07-18_multi-project-concurrency-and-citation-hygiene.md`。

### 后续项（未排期）

- **深度研究质量拉升专项（谭进 0717 反馈 #3 完整解）**：补强 S3 分析深度要求、S4 行业洞察 prompt 工程，并评估多轮 deepen 机制；另立专项 spec。
- **ChatPanel 池按需淘汰**：本批刻意采用会话内常驻；只有单会话项目数出现真实增长时，再基于数据设计淘汰策略。
- **计费 in-flight 预扣**：并发下日额度仍是软帽；当前量级接受，触发条件是出现可观测的并发超额。
- **跨进程并发**：A8 的 RLock/锁注册表只覆盖生产现状的单进程单 worker；若扩 worker，先引入跨进程锁与共享状态。
...
最后更新：2026-07-17（**目录固化架构重做 ✅：LO 降级为「页码 oracle」+ Python 写 Word 原生形态目录，修用户实测两 bug（WPS 点目录报错 + 目录后空白页）；Codex 审 APPROVED（3 NIT 已修）+ 部署 kr-web-01（第十三笔）重启；待用户 WPS/Word 复验**）：

**目录固化重做（2026-07-17，替代 07-16 首版）**：用户实测 07-16 固化版暴露两 bug——① WPS 点目录条目弹「无法打开指定的文件」；② 目录后多一页只有分页符的空白页。服务器解剖固化前后 XML 定位共同根因＝**LibreOffice 的 docx 导出回写**：目录条目写成 `__RefHeading___TocNNN` 私有锚点 + 7 个未闭合书签 + 大量重名书签（非法 OOXML，WPS 书签表解析失败→锚点当文件路径打开）；首个正文标题段被塞显式 `<w:br w:type="page"/>` 与 Heading2 样式 pageBreakBefore 双重分页（空白页 + 目录页码整体偏 1）；settings updateFields 被丢。**修法＝架构重做**：`lo_fixate.py` 只当页码 oracle（UNO 内存 `update()` 目录 → 读 `getAnchor().getString()` 解析「条目\t页码」→ 输出 JSON，**绝不 storeToURL**）；`report_tools._fixate_toc_fields` 在原始 docx 上字符串手术——Heading2-4 段注入 `_Toc9XXXXXXXX` 书签 + 占位 TOC 域段替换为 **Word F9 同构**静态条目（hyperlink `w:anchor` + `w:history` + PAGEREF 域缓存页码，H2→TOC1/H3→TOC2/H4→TOC3），docx 标题清单与 oracle 条目按数量+逐条文本对账、任何不匹配/坏 JSON/页码越界降级保留 TOC 域；文档其它 part 字节级不动（图表/页脚 PAGE 动态域/updateFields 天然保留，Word 打开仍整体重算、WPS 显示静态值且 F9 也能正确刷新）。**Codex 单轨审首轮即 APPROVED 零 BLOCKER**，3 条 NIT 已修（目录显示文本改用 docx 标题原文、fixture 补 pandoc 真实书签形态+书签体系语义断言、补 ZIP 写回失败/非 document part 字节级保留测试）+ 复核确认。后端 1821 passed；服务器 e2e 18 项结构断言全过 + **LO 转 PDF 渲染级校验**（7 页无空白页、目录声称页码与各章实际落页逐条一致）。**部署（第十三笔，2026-07-17）**：file-push report_tools+lo_fixate（sha256 双侧核验）+ systemd 重启 + 公网 health 200 + journal 干净 + 零 soffice 残留；回滚点 `/opt/cra-rollback-20260717/backend/`（07-16 版两文件）。硬约束记 CLAUDE.md「## docx 导出排版模板」→「### 目录固化」段（**绝不回退「LO 导出文档」路线**）。**待办**：用户 WPS/Word 复验修复版样例（目录可点击跳转、无报错弹窗、无空白页）。
