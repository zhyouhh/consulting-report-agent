...
test("shouldShowButton: lint_report removed (N7) — never visible", () => {
  for (const stageCode of ["S0", "S1", "S2", "S3", "S4", "S5", "S6", "S7", "done"]) {
    assert.equal(shouldShowButton("lint_report", stageCode), false);
  }
});
...
test("shouldShowButton: independent_review highlights when not ready", () => {
...
test("shouldShowButton: independent_review disabled when running", () => {
