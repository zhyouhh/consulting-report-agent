...
test("App createProject switches to the new project through attemptLeave guard (codex BLOCKER 1)", () => {
  ...
  // 创建成功后，乐观选择 + 静默对账都在 proceed 内，仍由 attemptLeave 决定执行时机。
  assert.match(s, /const proceed = async \(\) => \{[\s\S]*?setCurrentProjectId\(createdProject\.id\)[\s\S]*?refreshProjectsSilently\(createdProject\.id\)[\s\S]*?\}/);
  assert.match(s, /wp\.attemptLeave\(proceed\)/);
});
